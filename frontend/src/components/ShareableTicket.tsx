  ...
  const handleShare = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        // A) MÓVIL: Usar Share Nativo
        if (isMobile() && navigator.canShare && navigator.canShare({ files: [new File([blob], 'cotizacion.png', { type: 'image/png' })] })) {
          try {
        ...
        // B) PC: Portapapeles + Link
        try {
          const item = new ClipboardItem({ 'image/png': blob });
          await navigator.clipboard.write([item]);

          // Guardamos el link para abrirlo al cerrar el modal
          setPendingWaLink(waLink);

          const msgExito = hasValidPhone 
            ? `✅ Imagen copiada.\nSe abrirá WhatsApp con ${client.nombre} (${client.numero_referencia}).\nSolo presiona PEGAR (Ctrl + V).`
            : `✅ Imagen copiada.\nEl cliente no tiene un número válido, deberás elegir el contacto en WhatsApp.\nSolo presiona PEGAR (Ctrl + V).`;
          ...
          console.error(err);
          showError('Tu navegador bloqueó el copiado automático. Por favor, haz una captura de pantalla manual.');
        }
      ...
      console.error(err);
      showError('Error inesperado en el proceso.');
    }
  };
    ...
    <div style={{ marginTop: '10px', width: '100%' }}>
      ...
      <button 
        ...
        style={{ 
          ...
          backgroundColor: (client && client.nombre) ? '#25D366' : '#9ca3af', // Gris si inválido
      ...
      >
        ...
        {isSharing ? 'Generando...' : 'Enviar Cotización'}
      </button>
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom: '25px', backgroundColor: '#fff', padding:'15px', borderRadius: '8px', border: '1px solid #e5e7eb', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
             {result.imagen_procesada ? (
               <img src={result.imagen_procesada.startsWith('data:') ? result.imagen_procesada : `data:image/jpeg;base64,${result.imagen_procesada}`} alt="Diseño" style={{ maxHeight: '220px', maxWidth: '100%', objectFit: 'contain' }} />
